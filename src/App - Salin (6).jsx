...
function AuthContainer({ authPage, setAuthPage, onLoginSuccess, usersList, setUsersList }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      alert("✅ Registrasi berhasil! Saldo default: Rp0, Role: User.");
  ...
  const handleLogin = async () => {
    ...
    try {
      const userFound = usersList.find(u => u.no_hp === noHp && u.password === password);
      if (userFound) {
      ...
      setErrorMsg("Terjadi kendala saat login.");
    } finally {
      setLoading(false);
    }
  };
