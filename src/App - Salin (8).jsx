...
function AuthView({ authPage, setAuthPage, onLoginSuccess }) {
  ...
  // Handler Register dengan Pengecekan WA Unik (Hanya 1 WA)
  const handleRegister = async () => {
    if (!nama || !noHp || !password) return setErrorMsg("Mohon lengkapi seluruh kolom formulir.");
    setLoading(true);
    setErrorMsg("");
    ...
    try {
      // 1. Cek apakah nomor WA sudah pernah terdaftar di database
      const q = query(collection(db, "users"), where("no_hp", "==", cleanWa));
      const snap = await getDocs(q);

      if (!snap.empty) {
        setLoading(false);
        return setErrorMsg("Nomor WhatsApp ini sudah terdaftar. Silakan langsung masuk.");
      }

      // 2. Jika unik, simpan akun baru
      const newUser = {
        nama,
        no_hp: cleanWa,
        password,
      ...
      alert(" Pendaftaran berhasil! Saldo awal: Rp0, Role: User.");
